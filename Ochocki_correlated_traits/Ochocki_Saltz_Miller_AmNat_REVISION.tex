...
%Again, I don't think this is critical to address at this stage, though we can add this content if reviewers request it.
...
\revise{
We found additive genetic variation in demography and dispersal traits, though genetic and maternal variances were small in magnitude relative to environmental variances (Table \ref{corr:estimates}). 
Dispersal distance and fertility exhibited a similar amount of totaly phenotypic variance (the sum of the variance components) and a similar fraction of it was attributed to additive genetic effects.
There was a negative correlation (Pearson's $r = -0.118$) between raw phenotypic values of dispersal distance and fertility (averaged over variation bean densities) (Fig. \ref{corr:post}A), indicating that long-distance dispersers tended to produce fewer offspring.
...
%I am now thinking this is not necessary, though we can provide this if reviewers ask for it.
...
Our work moves beyond these familiar expectations for trait evolution to show, for the first time, how the sign, magnitude, and type of correlation between demography and dipsersal affects higher-level, ecological outcomes of range expansion, specifically speed and variability across realizations.
...
\linelabel{mod14}We found that evolutionary increases \revise{in} mean and variance were expected for \textit{C. maculatus} regardless of trait correlations: even a strict genetic trade-off ($\rho_{G} = -1$) could not have prevented evolutionary acceleration\revise{, a result anticipated by recent theory \citep{phillips2018spatial}}.
...
This recent is anticipated by recent theory on the dual roles of reproduction and movement in
Importantly, our simulations revealed that a strongly negative demography-dispersal genetic correlation, combined with a large amount of heritable variation, can actually \textit{decelerate} invasion (Fig. \ref{corr:sim_vary_h2_Pvar}).
...
\linelabel{mod15}\revise{The accumulation of deleterious mutations at expanding fronts (`expansion load') is another process that can slow down invasions \citep{gilbert2017local,peischl2018evolution}.
}
...
\revise{
Perhaps not surprisingly, the role of genetic correlations as a decelerating or accelerating factor depended on the amounts of heritable variation in demography and dispersal traits (Fig \ref{corr:sim_vary_h2_Pvar}).
...
This was likely the case in our experiments and simulations, though we did not attempt to quantify such changes.
}
...
These results point to \revise{a combination of three for} reasons why we failed to detect any evolved change in fertility in our previous invasion experiment: \revise{the relatively low additive genetic variance in fertility,} the pre-dominance of spatial \revise{sorting of} dispersal over \textit{r}-selection on fertility, and the negative genetic correlation between the two \revise{traits}, which further limited any evolved increase \revise{in fertility}.
