...
Spatial sorting generates an over-representation of highly dispersive phenotypes at the invasion vanguard\revise{, essentially the spatial analogue of classical natural selection in time \citep{phillips2018spatial}}.
...
\revise{
\linelabel{mod2}Only three previous studies, to our knowledge, have explored eco-evolutionary dynamics of invasion under trait correlations, with a focus on trade-offs (negative correlation) between life history and dispersal traits.
...
These studies made the important step of incorporating trait covariance into models of range expansion but, because they imposed a particular type of trade-off (negative, strong, genetic), they do not reveal the more general consequences of variation in the sign, magnitude, and type (genetic or environmental) of trait correlations.
Also, previous studies of demography-dispersal trade-offs have focused on trait evolution druing spread but not on the ecological outcomes of invasion speed and variability.}
Although there is an expectation that evolutionary processes can make invasions more variable \citep{phillips_evolutionary_2015,ochocki_rapid_2017,weiss-lehman_rapid_2017}, it is not clear how genetic and/or environmental correlations interact with other evolutionary processes to influence variability in invasion outcomes.
...
% We resolved that it was likely due to fewer mates provided to females in the breeding experiment than in the previous study.
...
\begin{figure}[h!]
\centering
\includegraphics[width=0.8\linewidth]{Figures/trait_biplots.pdf}
\caption{\textbf{Trait measurements and fitted genetic, maternal, and environmental components.}
\textit{A}, Raw observations for bean beetle dispersal distance and fertility (female offpsring per female per bean); phenotypic correlation $ = -0.12$ ($p < 0.004$).
\textit{B}, Fitted breeding values (additive genetic effects) for dispersal ($a^{d}_{jk}$) and fertility ($a^{r}_{jk}$), corresponding to Equation \ref{corr:gen}; fitted genetic correlation: $\rho_{G} = -0.17$.
\textit{C}, Fitted maternal effects for dispersal ($m^{d}_{k}$) and fertility ($m^{r}_{k}$), corresponding to Equation \ref{}; fitted correlation of maternal effects: $\rho_{M} = -0.26$.
\textit{D}, Fitted environmental devites for dispersal ($e^{d}_{i}$) and fertility ($e^{r}_{i}$), corresponding to Equation \ref{corr:env}; fitted correlation of environmental deviates: $\rho_{M} = -0.18$.
Additive genetic, maternal, and environmental effects are assumed to be multivariate-normally distributed with mean zero and variances and covariances given in Table \ref{corr:estimates}.
}\label{corr:biplots}
\end{figure}
