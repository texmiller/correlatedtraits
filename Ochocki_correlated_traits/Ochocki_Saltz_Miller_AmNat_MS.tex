...
\end{equation}
%
\tom{I think there is an error in this equation, should be $\frac{r-1}{K}\frac{N_{t}}{B}$ in the denominator. I tried checking Brad's code to see what he was actually fitting, but the stan code fits the B-H model with DD parameter b, not breaking out K. From the data and code available to me, I don't know where Brad's K estimates come from.}
...
For both covariances, genetic and environmental correlations were derived as $\rho = \frac{C}{\sqrt{V_{d}}\sqrt{V_{r}}}$. \tom{Check with Brad. I assume this is where the heritabilities in Figure \ref{corr:posteriors} come from.}
...
The initial conditions of the simulation mimic a small founding population being introduced to a novel landscape from some genetically well-mixed source population. Complete details of the simulations are provided in Online Appendix A.
...
Finally, we quantified trait evolution in the simulations by comparing the genotypes for both quantitative traits (population mean $\bm{\mu}$ plus breeding value $\bm{a}_{jk}$) between the starting value in the center patch and the final value at the range-edge patch.
...
We replicated the full range of trait correlations across five cases corresponding to heritability of dispersal and fertility:
\begin{itemize}
...
\caption[Estimates of key parameters from the beetle experiment]{\textbf{Estimates of key parameters from the beetle experiment.} Parameter estimates are grouped by the demographic process that they correspond to: dispersal, fertility, or both. Percentages show parameter estimates in the 95\% credible interval and the median (50\%) parameter estimate.\tom{Need to check with Brad about interpretation of phenotype values. For example mean dispersal phenotype is 2.74. If I am correct that this is log offspring, then this equals 15.48 offspring, which is much less than the average in OandM Figure 4. However, maybe it is female offspring, which would make the value more reasonable.}}\label{corr:estimates}\vspace{0.1in}
...
There were similar amounts of total phenotypic variance in the latent traits corresponding to dispersal and fertility ($V_{P,d}$  = 0.40, $V_{P,r}$  = 0.35; Fig. \ref{corr:posteriors}, Table \ref{corr:estimates}), and both exhibited additive genetic variance, indicating that both traits are heritable from parents to offspring.
...
\tom{[My thought here is that we can quantify how much of the posterior is negative vs positive, and this gives us a sense of confidence in the conclusion that correlations are negative.]}
...
\caption{\textbf{Traits.}}
\label{corr:traits}
\end{figure}

% Discussion -------------
\section*{Discussion}
Long-standing ecological theory and more recent eco-evolutionary work emphasize the key roles of demography and dispersal traits and the evolutionary forces that act on them as drivers of invasion dynamics but, in both classes of theory, regeneration and movement are typically assumed to operate independently.
On the other hand, different bodies of literature emphasize connections between demography and dispersal traits through a myriad of mechanisms, ranging from trade-offs and costs of dispersal (negative corrrelations) to dispersal `syndromes' that package several life history and movement traits into a multivariate phenotype (typically positive correlations).
Our work unifies both ends of this continuum into a cohesive framework for understanding the role of correlations in invasion dynamics, and carries significance at two levels.
...
Second, the generalized model analysis expanded the breadth of insight, revealing which outcomes were particular to the trait architecture of \textit{C. maculatus} and which were more general. 
...
Thus, the negative genetic correlation between dispersal and fertility in \textit{C. maculatus} (Fig. \ref{corr:posteriors}) caused the evolutionary increases in invasion speed and variability to be weaker than they would have been under no, or especially positive, genetic correlations (Fig. \ref{corr:sim_default}). 
...
The accelerating effect of positive correlations may also reflect some contribution of `enhanced spatial selection', whereby greater reproductive rates at the leading edge strengthen spatial selection on dispersal \citep{perkins_evolution_2013}. 
On the other hand, negative correlations constrain evolutionary acceleration of invasion, since an evolved increase in one trait would mean an evolved decrease in the other. 
...
Under a positive genetic correlation, the major axis phenotypic variance spans low-dispersal / low-fertility (and thus low invasion speed) to high-dispersal / high-fertility (and thus high invasion speed).
...
Conversely, under a negative genetic correlation, the major axis phenotypic variance spans low-dispersal / high-fertility to \textit{vice versa}; opposite ends of this axis are phenotypically different but similar in their resulting invasion speeds, due to the counter-acting effects of the two traits.
...
Our trait evolution results also shed light on results elsewhere in the literature, including evidence for evolved increases in dispersal but decreases in fertility in range-edge populations \citep{simmons_changes_2004,hughes_evolutionary_2003}. 
...
Positive phenotypic correlations that cause high-dispersal / high-fertility trait values to co-occur within individuals will always have a positive effect on the speed of invasion, and \textit{vice versa} for negative phenotypic correlations, regardless of whether or not the trait correlation is genetically based.
In this sense, genetic and environmental correlations were qualitatively interchangeable (Fig. \ref{corr:sim_default}). 
...
\paragraph{Genetic and non-genetic covariances are not interchangeable}

Our results also revealed that the asymmetry in trait evolution during spread was, to some extent, a consequence of the negative genetic correlation between fertility and dispersal (Fig. \ref{corr:traits}). 

\paragraph{Biological basis for trait correlations?}
\paragraph{Caveats: maternal effects, spatially uniform environment}
...
\caption{\textbf{Evolutionary effects on simulated \textit{C. maculatus} invasions.} Bars show the natural logarithm of fold-change due to evolution in invasion metrics and life history traits for simulation results corresponding to empirical estimates for \textit{C. maculatus} Table \ref{corr:estimates}.
White bars show the mean and CV of final extent after 20 generations of spread, and the fold-change is relative to `control' invasions with no genetically based trait variation ($h^{2}_{d} = h^{2}_{r} = 0$).
Black bars show genotypes underlying quantitative traits for dispersal distance ($\mu^{d} + a^{d}$) and low-density fertility ($\mu^{r} + a^{r}$), and fold-change compares the mean range-edge genotype in generation 20 with the initial genotype in generation 1.}
