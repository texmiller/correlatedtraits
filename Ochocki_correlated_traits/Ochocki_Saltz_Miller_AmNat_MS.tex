...
We then generalized those results by exploring realistic variation in trait correlations, heritability, and phenotypic variance.
...
We found that \textit{C. maculatus} exhibited similar amounts of total phenotypic variance in the latent traits corresponding to dispersal and fertility ($V_{P,d}$  = 0.40, $V_{P,r}$  = 0.35; Figure \ref{corr:posteriors}, Table \ref{corr:estimates}).
Furthermore, dispersal and fertility both exhibited additive genetic variance, indicating that both traits are heritable from parents to offspring.
...
Simulations of eco-ecolutionary spread dynamics using empirical estimates for genetic and environmental variation in demography and dispersal generated results that were consistent with previous studies of \textit{C. maculatus} invasions (Figure \ref{corr:barplot}). 
...
More positive genetic correlations led to stronger evolutionary acceleration, since selection could then favor high-dispersal / high-fertiltiy phenotypes. 
...
This was likely because invasions were already accelerated by positive environmental correlations, and there was therefore less opportunity for further increases in speed due to trait evolution. 
...
The evolutionary decrease in invasion speed occurred only when genetic correlations were very negative, where the best dispersers at the leading edge are very likely to carry low-fertility genotypes. 
...
Simulation results for evolved trait changes provide a mechanistic basis for the invasion metrics described above, and help contextualize the trait changes observed in the \textit{C. maculatus} system. 
